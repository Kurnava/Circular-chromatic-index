#ifndef BA_GRAPH_INVARIANTS_CIRCULAR_CVD_HPP
#define BA_GRAPH_INVARIANTS_CIRCULAR_CVD_HPP
...
/**
 * @brief Internal structure representing an edge for the Circular CVD solver.
 *
...
/**
 * @brief Internal graph representation optimized for the Circular CVD solver.
 *
...
struct FCVDInternalGraph {
    ...
    explicit FCVDInternalGraph(int n_)
        : n(n_),
          inc(n_) {}
...
/**
 * @brief Heuristic solver for circular edge colouring using randomized local search.
 *
...
public:
    /**
     * @brief Constructs the CircularCVD solver.
     *
     ...
     * @param max_steps The maximum number of iterations for the local search before giving up
     * (default 1,000,000).
     */
    CircularCVD(int p, int q = 2, int max_steps = 1'000'000)
        : p_(p),
          q_(q),
          max_steps_(max_steps),
          rng_(std::random_device{}()) {
        if (p_ <= 0) {
            throw std::runtime_error("CircularCVD: p must be positive");
        }
        if (q_ <= 0) {
            throw std::runtime_error("CircularCVD: q must be positive");
        }
        if (max_steps_ < 0) {
            throw std::runtime_error("CircularCVD: max_steps must be non-negative");
        }
     ...
     * @return false if the solver failed to find a valid colouring (the partial result is still
     * returned in colouring).
     */
    [[nodiscard]] bool run(const Graph &G, std::map<Edge, int> &colouring) {
        ...
        std::vector<int> vertex_to_idx(max_number(G).to_int() + 1, -1);
        int idx_counter = 0;
...
private:
    ...
    inline int rand_color() { return std::uniform_int_distribution<int>(0, p_ - 1)(rng_); }
     ...
     */
    bool creates_conflict_at(const FCVDInternalGraph &G, int u, int e_id, int new_color) const {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            for (size_t i = 0; i < L.size(); ++i) {
                for (size_t j = i + 1; j < L.size(); ++j) {
                    ...
                    if (cyclic_dist(G.edges[e1_id].color, G.edges[e2_id].color, p_) >= q_) {
                        continue;
                    }
                    ...
                    int v = (G.edges[e_id].u_idx == u) ? G.edges[e_id].v_idx : G.edges[e_id].u_idx;
